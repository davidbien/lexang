...
template < class t_TyNfa, class t_TyDfa > struct _create_dfa
{
private:
...
protected:
  ...
  typedef unordered_map< _TySwapSSNfa, _TyGraphNodeDfa *, hash< _TySwapSSNfa >, equal_to< _TySwapSSNfa >, _tySwapSSNfaAlloc > _TyLookupSS;
  ...
  typedef typename _Alloc_traits< typename map< _TyPairSSActionIds, _TyPairStatesAction, _TyCompareAmbigKey >::value_type, _TyAllocatorNfa >::allocator_type
      _TyAmbigAcceptAlloc;
...
public:
  _create_dfa( _TyNfa & _rNfa, _TyNfaCtxt & _rNfaCtxt, _TyDfa & _rDfa, _TyDfaCtxt & _rDfaCtxt, bool _fCreateDeadState, size_t _stHashSize = 3000 )
    : m_rNfa( _rNfa )
    , m_rNfaCtxt( _rNfaCtxt )
    , m_rDfa( _rDfa )
    , m_rDfaCtxt( _rDfaCtxt )
    , m_sCur( 0 )
    , m_ssLookup( _stHashSize, typename _TyLookupSS::hasher(), typename _TyLookupSS::key_equal(), _rNfa.get_allocator() )
  ...
  bool create()
  {
    ...
    // We can however, in the presence of at least push_back() or push_front(),  store pointers/refs to elements. In fact as such we can just store an index to
    // the element in question. Get the accepting states from the NFA:
    ...
    auto fcReleaseAccept = [ this, stReleaseAccepting ]() // define the lambda for the releasing pssAccepting back to the cache.
    ...
    CMFDtor1_void< _TyNfa, size_t > releaseCur( &m_rNfa, &_TyNfa::_ReleaseSSCache, m_rNfa._STGetSSCache( m_pssCur ) );
    m_pssCur->clear();
    ...
    CMFDtor1_void< _TyNfa, size_t > releaseMove( &m_rNfa, &_TyNfa::_ReleaseSSCache, m_rNfa._STGetSSCache( pssMove ) );
    pssMove->clear();
    ...
    while ( m_sCur != m_rDfa.NStates() )
    {
      ...
      _TyGraphLinkDfa * pglFirstAdded = ( m_rNfa.m_nTriggers || m_rNfa.m_nUnsatisfiableTransitions ) ? 0 : (_TyGraphLinkDfa *)1;
      ...
      do
      {
        ...
        if ( !m_pssCur->empty() )
        {
          ...
          if ( itLookup == m_ssLookup.end() )
          {
            ...
            m_rDfa._NewAcceptingState( pgnCurDfa, aiCur, &pgnNew, pglFirstAdded ? 0 : &pglFirstAdded );
            (void)_NewDfaState( pgnNew );
          }
          else
          {
            m_rDfa._NewTransition( pgnCurDfa, aiCur, ( *itLookup ).second, pglFirstAdded ? 0 : &pglFirstAdded );
          }
        }
        else
        {
          ...
          if ( m_fCreateDeadState )
          {
            ...
            m_rDfa._NewTransition( pgnCurDfa, aiCur, pgnDead, pglFirstAdded ? 0 : &pglFirstAdded );
          }
        }
    ...
    if ( m_fCreateDeadState && pgnDead->FParents() )
    {
      ...
      for ( size_t stUnsat = 0; stUnsat++ < m_rNfa.m_nUnsatisfiableTransitions; )
      {
        m_rDfa._NewTransition( pgnDead, ( typename _TyDfa::_TyAlphaIndex )( m_rDfa.m_setAlphabet.size() - stUnsat ), pgnDead, 0 );
      }

      for ( size_t stTrigger = 0; stTrigger++ < m_rNfa.m_nTriggers; )
      {
        // First transition should be trigger transition:
        m_rDfa._NewTransition(
            pgnDead, ( typename _TyDfa::_TyAlphaIndex )( m_rDfa.m_setAlphabet.size() - stTrigger - m_rNfa.m_nUnsatisfiableTransitions ), pgnDead, 0 );
      }
    }
  ...
  _TySetStatesNfa const * _NewDfaState( _TyGraphNodeDfa * _pgn )
  {
    ...
    if ( m_pssAcceptingNfa->size() != stFirst )
    {
      ...
      if ( stNext != m_pssAcceptingNfa->size() )
      {
        ...
        typename _TyAcceptAction::_TySetActionIds srAction( m_rDfa.m_iMaxActions, rvtFirst.second.get_allocator() );
        srAction.clear();
        srAction.setbit( rvtFirst.second.m_aiAction );
        srAction.setbit( m_rNfa.m_pSetAcceptStates->find( stNext )->second.m_aiAction );

        while ( ( stNext = m_pssAcceptingNfa->NextIntersection( *m_pssCur, stNext ) ) != m_pssAcceptingNfa->size() )
        {
          srAction.setbit( m_rNfa.m_pSetAcceptStates->find( stNext )->second.m_aiAction );
        }
        ...
        typename _TyAcceptAction::_TySetActionIds srFoundLARelations( m_rDfa.m_iMaxActions, rvtFirst.second.get_allocator() );
        srFoundLARelations.clear();

        // Bit vector of found trigger action ids:
        bool fFoundTrigger = false;
        typename _TyAcceptAction::_TySetActionIds srFoundTriggers( m_rDfa.m_iMaxActions, rvtFirst.second.get_allocator() );
        ...
        for ( vtyActionIdent aiAction = aiActionMin; srAction.size() != aiAction; aiAction = (vtyActionIdent)srAction.getclearfirstset( aiAction ) )
        {
          Assert( srAction.size() != aiAction );
          typename _TyNfa::_TySetASByActionID::iterator it = m_rNfa.m_pLookupActionID->find( aiAction );
       ...
       // If we found a lookahead accept node that was ambiguous then don't use it:
          ...
          if ( fFoundTrigger )
          {
            ...
            if ( srFoundTriggers.getnextset( aiFirst ) != srFoundTriggers.size() )
            {
              ...
              do
              {
                typename _TyNfa::_TySetASByActionID::value_type & rvtFirst = *m_rNfa.m_pLookupActionID->find( aiFirst );

                for ( vtyActionIdent aiNext = aiFirst; srTriggers.size() != ( aiNext = (vtyActionIdent)srTriggers.getnextset( aiNext ) ); )
                {
                  if ( m_rNfa.m_pLookupActionID->find( aiNext )->second->second.m_pSdpAction->GetBaseP()->FIsSameAs(
              ...
              } while ( srTriggers.size() != ( aiFirst = (vtyActionIdent)srTriggers.getclearfirstset( aiFirst ) ) );
            }
            ...
            if ( !fFoundLookaheadAccept && !fMultiTrigger && !pvtFirstAccept )
            {
              ...
              typename _TyNfa::_TySetASByActionID::value_type & rvtMin = *m_rNfa.m_pLookupActionID->find( aiMin );
              m_lDfaAccepting.push_front( typename _TyDfaAcceptingList::value_type( rvtMin.second->first, _pgn->RElConst() ) );
              return pssInLookup;
            }
          }
          ...
          if ( m_mapAmbigAccept.end() == itAmbig )
          {
            _TyAcceptAction aaNew(
                pvtFirstAccept ? pvtFirstAccept->second.m_aiAction : m_rNfa.m_iActionCur++, pvtFirstAccept ? pvtFirstAccept->second.m_pSdpAction.Ptr() : 0 );
            if ( fFoundLookaheadAccept )
            {
              if ( pvtFirstAccept )
              {
                Assert( e_aatAntiAccepting != aaNew.m_eaatType );
                aaNew.m_eaatType = e_aatLookahead == pvtFirstAccept->second.m_eaatType ? e_aatLookaheadAcceptAndLookahead : e_aatLookaheadAcceptAndAccept;
              }
              else
              {
            ...
            if ( fFoundLookaheadAccept )
            {
              itAmbig->second.second.m_psrRelated
                  .template emplace< typename _TyAcceptAction::_TySetActionIds::size_type, typename _TyAcceptAction::_TySetActionIds::_TyAllocator const & >(
                      0, srFoundLARelations.get_allocator() );
            }
            if ( fFoundTrigger )
            {
              itAmbig->second.second.m_psrTriggers
                  .template emplace< typename _TyAcceptAction::_TySetActionIds::size_type, typename _TyAcceptAction::_TySetActionIds::_TyAllocator const & >(
                      0, srFoundLARelations.get_allocator() );
            }
          }
      ...
      m_lDfaAccepting.push_front( typename _TyDfaAcceptingList::value_type( stFirst, _pgn->RElConst() ) );
    }
    return pssInLookup;
  }
  ...
  void _ConstructAcceptPartition()
  {
    ...
    for ( typename _TyDfaAcceptingList::iterator itL = m_lDfaAccepting.begin(); itL != itLEnd; ++itL )
    {
      ...
      if ( mapNfaState.end() == ( itNfaState = mapNfaState.find( itL->first ) ) )
      {
        ...
        _TySetStatesDfa ssDfa( static_cast< size_t >( m_rDfa.NStates() ), m_rDfa.get_allocator() );
        ssDfa.clear();
    ...
    {                      //	reject states.
      ...
      for ( ; itAmbigEnd != itAmbig; ++itAmbig )
      {
        ...
        *m_rDfaCtxt.m_pssAccept |= ssDfa;
        ...
        {
          pib.first->second.m_psrRelated->swap( const_cast< typename _TyAcceptAction::_TySetActionIds & >( rvtAmbig.first.first.RObject() ) );
        }

        // Get the trigger vector from the key:
        if ( rvtAmbig.second.second.m_psrTriggers )
        {
          pib.first->second.m_psrTriggers->swap( const_cast< typename _TyAcceptAction::_TySetActionIds & >( rvtAmbig.first.second.RObject() ) );
        }
      }
    }
    ...
    for ( itNfaState = mapNfaState.begin(); itNfaStateEnd != itNfaState; ++itNfaState )
    {
      typename _TyPartAcceptStates::value_type vt( itNfaState->second.first, itNfaState->second.second );
#ifndef NDEBUG
    ...
    if ( m_fAmbiguousTriggers )
    {
      ...
      m_rDfa.m_pMapTriggers.template emplace< typename _TyDfa::_TyCompareAI const &, typename _TyDfa::_TyAllocator const & >(
      ...
      for ( typename _TyNfa::_TySetAcceptIT itNfaAccept = m_rNfa.m_pSetAcceptStates->begin(); itNfaAccept != itNfaAcceptEnd; ++itNfaAccept )
      {
        if ( e_aatTrigger == itNfaAccept->second.m_eaatType )
        {
          typename _TyDfa::_TyMapTriggers::value_type vt( itNfaAccept->second.m_aiAction, itNfaAccept->second );
